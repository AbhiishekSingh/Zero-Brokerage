...
function Mumbai() {
  ...
  useEffect(() => {
    axios.get('/alldata.json')
      .then(response => {
        setRooms(response.data.mumbai);
        setFilteredRooms(response.data.mumbai); // Initialize filtered rooms with all rooms
      })
      .catch(error => {
  ...
  return (
    <div>
      <div class="Mumbai-BG">
        ...
        <div class="Mumbai-line">Mumbai's Rooms & Place to stay</div>
  ...
  {filteredRooms.map((x,index)=>{
    return(
      <Allrooms name={x.name} address={x.address} price={x.price} id={x.id}
       sqr={x.sqr} furnishing={x.furnishing} appartment={x.appartment} availablefrom={x.availablefrom} state={"mumbai"}/>
    )
  })} 
    </div>
    </div>
