...
function Hyderabad() {
  ...
  useEffect(() => {
    axios.get('/alldata.json')
      .then(response => {
        setRooms(response.data.hyderabad);
        setFilteredRooms(response.data.hyderabad); // Initialize filtered rooms with all rooms
      })
      .catch(error => {
  ...
  return (
    <div>
      <div class="Mumbai-BG">
        <img class="city-image" src="https://th.bing.com/th/id/OIP.F5yrDEhTvQeHJggS-hPqNgHaEK?rs=1&pid=ImgDetMain" alt="Mumbai" />
        <div class="Mumbai-line">Hyderabad's Rooms & Place to stay</div>
  ...
  <h1 className='mumbai-MH'>Rooms in Hyderabad, Telangana</h1>
  {filteredRooms.map((x,index)=>{
    return(
      <Allrooms name={x.name} address={x.address} price={x.price} id={x.id}
      sqr={x.sqr} furnishing={x.furnishing} appartment={x.appartment} availablefrom={x.availablefrom}  state={"hyderabad"}/>
    )
  })} 
    </div>
    </div>
