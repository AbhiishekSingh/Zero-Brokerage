...
function Srinagar() {
  ...
  useEffect(() => {
    axios.get('/alldata.json')
      .then(response => {
        setRooms(response.data.srinagar);
        setFilteredRooms(response.data.srinagar); // Initialize filtered rooms with all rooms
      })
      .catch(error => {
  ...
  return (
    <div>
      <div class="Mumbai-BG">
        ...
        <div class="Mumbai-line">Srinagar's Rooms & Place to stay</div>
  ...
  <h1 className='mumbai-MH'>Rooms in Srinagar, Jammu & Kashmir</h1>
  {filteredRooms.map((x,index)=>{
    return(
      <Allrooms name={x.name} address={x.address} price={x.price} id={x.id}
      sqr={x.sqr} furnishing={x.furnishing} appartment={x.appartment} availablefrom={x.availablefrom}  state={"srinagar"}/>
    )
  })} 
    </div>
    </div>
