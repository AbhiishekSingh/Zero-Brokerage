...
function Simla() {
  ...
  useEffect(() => {
    axios.get('/alldata.json')
      .then(response => {
        setRooms(response.data.shimla);
        setFilteredRooms(response.data.shimla); // Initialize filtered rooms with all rooms
      })
      .catch(error => {
  ...
  return (
    <div>
      <div class="Mumbai-BG">
        ...
        <div class="Mumbai-line">Shimla's Rooms & Place to stay</div>
  ...
  <h1 className='mumbai-MH'>Rooms in Shimla, Himanchal Pradesh</h1>
  {filteredRooms.map((x,index)=>{
    return(
      <Allrooms name={x.name} address={x.address} price={x.price} id={x.id}
      sqr={x.sqr} furnishing={x.furnishing} appartment={x.appartment} availablefrom={x.availablefrom}  state={"shimla"}/>
    )
  })} 
    </div>
    </div>
